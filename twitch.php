...
if(isset($_GET['channel'])) {
	$PRESET['channel']=preg_replace("/[^a-z0-9_]+/i","",$_GET['channel']);//if a channel was set remove all non-valid characters
} else {$PRESET['channel']="";}
if(isset($_GET['h'])) {
	$PRESET['h']=intval(preg_replace("/[^0-9]+/i","",$_GET['h']));//if a height was set remove all non-number characters and convert to int
	$PRESET['w']=intval($PRESET['h']/9*16);//pre-calculate the initial width for 16:9 aspectratio
} else {$PRESET['w']=1280;$PRESET['h']=720;}
$PRESET['chat']=3;
if(isset($_GET['chat'])&&$_GET['chat']=="left") $PRESET['chat']=1;
if(isset($_GET['chat'])&&$_GET['chat']=="nochat") $PRESET['chat']=2;
?>	<p>Enter channel name here:<br>
		<input type="text" name="popouturl" value="<?php echo $PRESET['channel'];?>" id="popouturl" style="width:500px"></p>
	<table>
		<tr>
			<td>Window width:</td>
			<td><input type="number" name="width" id="inputwidth" min="100" value="<?php echo $PRESET['w'];?>" oninput="aspectratio(this,'inputheight','keep169option',16);"><span id="chatsizenote" class="note">+340</span></td>
		</tr>
		<tr>
			<td>Window height:</td>
			<td><input type="number" name="height" id="inputheight" min="100" value="<?php echo $PRESET['h'];?>" oninput="aspectratio(this,'inputwidth','keep169option',9);"></td>
		</tr>
		<tr>
		...
		<div id="chatsidediv">
			<input type="radio" name="chatsidechoice" id="chatsideleft" value="left" onChange="ToggleChat('radio-1');"<?php if($PRESET['chat']==1) echo " checked";?>><label for="chatsideleft" class="is-radio"> Show chat on the left side</label>
			<input type="radio" name="chatsidechoice" id="chatsidenone" value="none" onChange="ToggleChat('radio-2');"<?php if($PRESET['chat']==2) echo " checked";?>><label for="chatsidenone" class="is-radio"> Don't show chat</label>
			<input type="radio" name="chatsidechoice" id="chatsideright" value="right" onChange="ToggleChat('radio-3');"<?php if($PRESET['chat']==3) echo " checked";?>><label for="chatsideright" class="is-radio"> Show chat on the right side</label>
			<div style="clear:both;" class="radio-<?php echo $PRESET['chat'];?>" id="chatsidebg"><div class="radio-bg"></div></div>
		</div>
